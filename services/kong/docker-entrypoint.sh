#!/usr/bin/env bash
# original file: https://raw.githubusercontent.com/Kong/docker-kong/2.1.0/alpine/docker-entrypoint.sh
set -Eeo pipefail
...
if [[ "$1" == "kong" ]]; then
  ...
  if [[ "$2" == "docker-start" ]]; then
    ...
    # Copy the custom nginx-kong.conf file
    cp /usr/local/kong/nginx-kong-modified.conf /usr/local/kong/nginx-kong.conf
